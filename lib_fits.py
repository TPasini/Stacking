...
class AllImages():
    ...
    def write(self, suffix, inflate=True):
        ...
        for image in self.images:
            ...
            # Inflate a fits file so that it becomes a 4D image. Return new header and data
                ...
                hdr_inf['CDELT4'  ] = 1.
                hdr_inf['CUNIT4'  ] = ' '
                pyfits.writeto(image.imagefile.replace('.fits', f'-{suffix}.fits'), self.img_data[np.newaxis,np.newaxis], hdr_inf, overwrite=True, output_verify='fix')
        #else:
